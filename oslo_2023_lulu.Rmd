...
To decide wether or not **A** is a potential parent of **B**, lulu
computes their relative co-occurence. For example:
...
co-occur in two samples (relative co-occurence of 2/3).

The relative co-occurence must be greater or equal to a threshold
...
and **B** is present in three samples, so the relative co-occurence
...
A relative co-occurence of 0.95 means that we need at least 20
...
Relative co-occurence is accepted, but the minimal observed abundance
ratio is null where **A** is absent (0 / 1 = 0), which leads to
rejection. So according to its documentation, `lulu` allows for
partial overlaps by default, but in reality they never pass the
abundance ratio filter.
...
difference between documentation and the way the programm actually
works). At the beginning of its developement, `mumu` was a `lulu`
...
- add: automatic tests,
- document: threshold limits (greater than, or equal or greater than?),
- document: minimum match threshold representation is not clearly stated (0 to 1, or 0 to 100) (mixed up examples),
...
- fix: make `lulu` faster (e.g.; other dependency, Rcpp)
...
its limitations. I would like to point out that programming bugs are a
**very** common thing, and that the presence of a bug in `lulu` should
not reflect badly on the work of Frøslev and colleagues.
